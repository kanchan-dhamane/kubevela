...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	conversionData, present := componentDefinition.ObjectMeta.Annotations["definition.oam.dev/conversion-data"]
	if present {
		var componentDefinitionBeta2 v1beta2.ComponentDefinition
		err := json.Unmarshal([]byte(conversionData), &componentDefinitionBeta2)
		...
		for _, version := range componentDefinitionBeta2.Spec.Versions {
			...
			newDef.Spec.PodSpecPath = version.PodSpecPath
			newDef.Spec.Status = version.Status
